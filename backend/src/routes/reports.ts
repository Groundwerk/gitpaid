...
function drawReportFooter(doc: jsPDF, userEmail: string, currentPage: number = 1, totalPages: number = 1) {
  ...
  doc.text('Generated by Gitpaid', pageWidth - 50, pageHeight - 45, { align: 'right' });
...
router.get('/net-pay', async (c) => {
  try {
    ...
    for (let i = 1; i <= totalPages; i++) {
      doc.setPage(i);
      drawReportFooter(doc, userEmail, i, totalPages);
    }
